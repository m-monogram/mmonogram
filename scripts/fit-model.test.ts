...
/* Фонарь и фара бывают только на торцах: тот же материал в середине машины
   это обломок прореживания, а не фонарь. Раньше он переставал светиться и
   уходил в trim — и оставался висеть тёмным лоскутом в проходе между
   рядами, прямо перед интерьерной камерой. Такие меши прячем. */
check(
...
// Покрышки идут одним мешем на все четыре колеса: длинный, но высокий — резина.
// Порог лежит на той же высоте, но низкий — не колесо.
check(
  "покрышки одним мешем",
  classifyPart(mat("Material.014", { metalness: 0.71, roughness: 0.58, luma: 0 }), new THREE.Box3(
    new THREE.Vector3(-1.9, 0.0, -1.0), new THREE.Vector3(1.9, 0.8, 1.0),
  ), CAR_SIZE, "Mesh.010"),
  "tire",
);

// Стекло по форме — только без материала: глухая панель прозрачной не станет
check(
  "тонкая панель с материалом",
  classifyPart(mat("mat_2.001", { luma: 0.2 }), new THREE.Box3(
    new THREE.Vector3(-0.35, 1.74, -0.21), new THREE.Vector3(0.35, 1.76, 0.21),
  ), CAR_SIZE, "Mesh.5802"),
  "trim",
);
...
const debrisCases: [string, number, [number, number, number], boolean][] = [
  ...
  ["плита из десяти треугольников на окно", 10, [1.54, 0.74, 2.65], true],
  ["перегородка на пятнадцати треугольниках", 15, [1.13, 1.24, 0.25], true],
