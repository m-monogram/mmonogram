...
/* Вертикальная растяжка циклорамы: почти белый горизонт, кверху уходит в
   средне-серый. Так снято у ателье вроде Mansory: машина стоит в световом
   пятне, а верх кадра приглушён и не спорит с ней. Прежние значения давали
   ровный серый по всей высоте — студия читалась как пасмурный день. */
const COVE_STOPS = {
  light: ["#eceef0", "#e7e9eb", "#d6dade", "#babfc5", "#989ea4"],
  dark: ["#23262a", "#1c1f22", "#15181b", "#101215", "#0a0b0d"],
} as const;

/* Пол: световое пятно под машиной, к краю уходит ровно в нижний стоп
   циклорамы (#eceef0) — иначе на горизонте читается ступенька яркости,
   и по кадру идёт дуга там, где круг пола встречается со скруглением. Раньше центр был темнее краёв, машина стояла
   в сером пятне вместо светового. Опору под ней держит не тёмный пол, а
   контактная тень (SoftGroundShadow), она рисуется поверх. */
const FLOOR_STOPS = {
  light: ["#fdfdfe", "#f5f6f7", "#eceef0"],
